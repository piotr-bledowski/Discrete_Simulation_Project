...
def on_click(event):
    if event.xdata is not None and event.ydata is not None:
        ...
        velocity_source[y, x, 0] += (cp.random.rand() - 0.5) * force_strength
        velocity_source[y, x, 1] += (cp.random.rand() - 0.5) * force_strength
        force_timer[y, x] = force_duration

# Plotting setup
fig, ax = plt.subplots()
plt.subplots_adjust(left=0.25, bottom=0.4)
im = ax.imshow(cp.asnumpy(grid[:, :, 0]), animated=True, cmap="plasma")
plt.colorbar(im)
...
slider_force = Slider(ax_force, 'Force Strength', 10, 1000, valinit=force_strength)
